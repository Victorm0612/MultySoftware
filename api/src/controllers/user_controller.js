...
export async function login(req, res){
    ...
    if(user){
        const token = jwt.sign({user}, process.env.JWT_SECRET, {expiresIn: '60s'}, { algorithm: 'HS256', expiresIn: '1h' })
        const refreshToken = jwt.sign({user}, process.env.JWT_REFRESH_SECRET, { algorithm: 'HS256' })
...
export async function refresh_token(req, res){
    try {
        ...
        if (user === null)
            res.status(403).json({ 'RES': 'Error token inválido.' })
        else {
            const token = jwt.sign({user}, process.env.JWT_SECRET, {expiresIn: '60s'}, { algorithm: 'HS256', expiresIn: '1h' })
            const refreshToken = jwt.sign({user}, process.env.JWT_REFRESH_SECRET, { algorithm: 'HS256' })
